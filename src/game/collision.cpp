...
static int GetMoveRestrictionsRaw(int Direction, int Tile, int Flags, CDDNetPPMoveRestrictionData *pDDNetPP)
{
	...
	case TILE_VIP_PLUS_ONLY:
		if(pDDNetPP && !pDDNetPP->m_CanEnterVipPlusOnly)
		{
			pDDNetPP->m_VipPluOnlyEnterBlocked = true;
			return CANTMOVE_LEFT | CANTMOVE_RIGHT | CANTMOVE_UP | CANTMOVE_DOWN;
		}
		break;
	case TILE_VIP_ONLY:
...
bool CCollision::TileExistsNext(int Index) const
{
	...
	if(m_pTiles[TileBelow].m_Index == TILE_ROOM || m_pTiles[TileAbove].m_Index == TILE_ROOM)
		return true;
	if(m_pTiles[TileOnTheRight].m_Index == TILE_VIP_PLUS_ONLY || m_pTiles[TileOnTheLeft].m_Index == TILE_VIP_PLUS_ONLY)
		return true;
	...
	if(m_pFront)
	{
		if(m_pFront[TileOnTheRight].m_Index == TILE_ROOM || m_pFront[TileOnTheLeft].m_Index == TILE_ROOM)
			return true;
		if(m_pFront[TileBelow].m_Index == TILE_ROOM || m_pFront[TileAbove].m_Index == TILE_ROOM)
			return true;
...
int CCollision::IntersectNoLaser(vec2 Pos0, vec2 Pos1, vec2 *pOutCollision, vec2 *pOutBeforeCollision) const
{
	...
	for(int i = 0; i < DistanceRounded; i++)
	{
		...
		bool DDNetPPConfigTile = (GetIndex(Nx, Ny) == TILE_CONFIG_1 &&
						 g_Config.m_SvCfgTile1 == CFG_TILE_UNHOOK) ||
					 (GetIndex(Nx, Ny) == TILE_CONFIG_2 && g_Config.m_SvCfgTile2 == CFG_TILE_UNHOOK);

		if(GetIndex(Nx, Ny) == TILE_SOLID || GetIndex(Nx, Ny) == TILE_NOHOOK || GetIndex(Nx, Ny) == TILE_NOLASER || GetFrontIndex(Nx, Ny) == TILE_NOLASER || DDNetPPConfigTile)
		{
			if(pOutCollision)
				*pOutCollision = Pos;
			if(pOutBeforeCollision)
