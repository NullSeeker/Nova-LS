...
void CGameContext::SendChat(int ChatterClientId, int Team, const char *pText, int SpamProtectionClientId, int VersionFlags, int ToClientId)
{
...
void CGameContext::SendMotd(int ClientId) const
{
	...
	// ddnet++
	// we show online supporters instead of default sv_motd
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(m_apPlayers[i] && m_apPlayers[i]->m_Account.m_IsSupporter)
		{
	...
	if(ClientId == -1) //classical rcon broadcast
	{
		...
		for(auto &pPlayer : m_apPlayers)
		{
			if(pPlayer)
			{
				...
				pPlayer->m_LastBroadcastImportance = 1;
	...
	else //non rcon broadcast
	{
		...
		if(m_apPlayers[ClientId]->m_LastBroadcastImportance) //only care if last broadcast was important
		{
			if(m_apPlayers[ClientId]->m_LastBroadcast > Server()->Tick() - Server()->TickSpeed() * 6) //dont overwrite broadcasts send 6 seconds ago
			{
				if(Importance == 0)
				{
					//SendChat(-1, TEAM_ALL, "broadcast got ignored");
					return;
				}
				else if(Importance == 1 && IsSupermod && m_apPlayers[ClientId]->m_LastBroadcastImportance == 2) //supermoderators can't overwrite broadcaste with lvl 2 importance
				{
					//SendChat(-1, TEAM_ALL, "broadcast got ignored");
					return;
				}
	...
	if(m_VoteCloseTime)
	{
		...
		else
		{
									...
									(IsKickVote() || IsSpecVote()) && time_get() < m_VoteCloseTime))
			{
				...
				EndVote();
				SendChat(-1, TEAM_ALL, m_IsDDPPVetoVote ? "Vote passed because nobody used veto (Veto Vote)" : "Vote passed", -1, FLAG_SIX);
...
void CGameContext::OnSetTeamNetMessage(const CNetMsg_Cl_SetTeam *pMsg, int ClientId)
{
	...
	if(AbortTeamChange(ClientId, pPlayer)) // ddnet++
		return;
...
void CGameContext::OnChangeInfoNetMessage(const CNetMsg_Cl_ChangeInfo *pMsg, int ClientId)
{
	...
	if(Server()->WouldClientNameChange(ClientId, pMsg->m_pName) && !ProcessSpamProtection(ClientId))
	{
		...
		// ddnet++
		int MuteTime = NameChangeMuteCheck(ClientId);
		if(MuteTime > 0)
		{
			str_format(aChatText, sizeof aChatText, "[MUTE] %d seconds delay for name change message.", MuteTime);
...
void CGameContext::OnEmoticonNetMessage(const CNetMsg_Cl_Emoticon *pMsg, int ClientId)
{
	...
	if(pPlayer->m_SpookyGhostActive) // ddnet++
	{
...
void CGameContext::OnKillNetMessage(const CNetMsg_Cl_Kill *pMsg, int ClientId)
{
	...
	if(AbortKill(ClientId, pPlayer, pChr)) // ddnet++
		return;
...
void CGameContext::RegisterDDRaceCommands()
{
	...
	Console()->Register("addweapon", "i[weapon-id] ?v[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConAddWeapon, this, "Gives weapon with id i to you or target id/all (all = -1, hammer = 0, gun = 1, shotgun = 2, grenade = 3, laser = 4, ninja = 5)");
	Console()->Register("removeweapon", "i[weapon-id] ?v[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConRemoveWeapon, this, "removes weapon with id i from you or target id/all (all = -1, hammer = 0, gun = 1, shotgun = 2, grenade = 3, laser = 4, ninja = 5)");
	Console()->Register("shotgun", "?v[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConShotgun, this, "Gives a shotgun to you");
	Console()->Register("grenade", "?v[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConGrenade, this, "Gives a grenade launcher to you");
	Console()->Register("laser", "?v[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConLaser, this, "Gives a laser to you");
	Console()->Register("rifle", "?v[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConLaser, this, "Gives a laser to you");
	Console()->Register("jetpack", "?v[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConJetpack, this, "Gives jetpack to you or");
	Console()->Register("setjumps", "i[jumps]", CFGFLAG_SERVER | CMDFLAG_TEST, ConSetJumps, this, "Gives you as many jumps as you specify");
	Console()->Register("weapons", "?v[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConWeapons, this, "Gives all weapons to you or");
	...
	Console()->Register("ninja", "?v[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConNinja, this, "Makes you a ninja");
	Console()->Register("unninja", "", CFGFLAG_SERVER | CMDFLAG_TEST, ConUnNinja, this, "Removes ninja from you");
	Console()->Register("super", "?v[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConSuper, this, "Makes you super");
	...
	Console()->Register("infinite_jump", "?v[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConEndlessJump, this, "Gives you infinite jump");
	Console()->Register("uninfinite_jump", "", CFGFLAG_SERVER | CMDFLAG_TEST, ConUnEndlessJump, this, "Removes infinite jump from you");
	Console()->Register("endless_hook", "?v[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConEndlessHook, this, "Gives you endless hook");
...
void CGameContext::RegisterChatCommands()
{
	...
	Console()->Register("vip", "", CFGFLAG_CHAT, ConVip, this, "Shows VIP status and VIP commands");
	...
	Console()->Register("weapons", "", CFGFLAG_CHAT, ConPracticeWeapons, this, "Gives all weapons to you (VIP, 3 minute cooldown)");
...
void CGameContext::OnSetAuthed(int ClientId, int Level)
{
	if(m_apPlayers[ClientId] && m_VoteCloseTime && Level != AUTHED_NO)
	{
		...
		str_format(aBuf, sizeof(aBuf), "auth_level=%d %sip=%s name=%s", Level, aAccId, Server()->ClientAddrString(ClientId, false), Server()->ClientName(ClientId));
